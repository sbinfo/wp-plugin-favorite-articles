...
function sbg_favorites_admin_scripts($hook) {
	if ($hook != 'index.php') {
		return;
	}
	...
	wp_localize_script('sbg-dashboard-scripts', 'sbgFavorites', ['nonce' => wp_create_nonce('sbg-admin-favorites')]);
}
...
function sbg_show_dashboard_widget() {
	...
	echo '<ul class="sbg-favorites-list">';
	foreach ($favorites as $item) {
		$post_image = get_the_post_thumbnail($item, array(35, 35));
		if (!$post_image) {
			$post_image = '<img src="' . plugins_url('/img/no-image.png', __FILE__) . '" alt="">';
		}
		echo '<li class="sbg-dashboard-favorites sbg-dashboard-favorites-item-' . $item . '">
			<a href="#">
				<img src="' . $loader_src . '" class="sbg-favorites-loading-' . $item . ' sbg-favorites-hidden" alt="">
			</a>
			<a class="hide" href="' . get_permalink($item) . '">' . $post_image . '</a>
			<a class="hide" href="' . get_permalink($item) . '">' . get_the_title($item) . '</a>
			<span class="hide delete-item-btn" data-id="' . $item . '">
			<i data-action="delete" class="fas fa-trash-alt"></i>
			</span>
		</li>';
	}
	echo '<li class="sbg-dashboard-favorites-loader">
			<img src="' . $loader_src . '" class="sbg-favorites-hidden" alt="">
		</li></ul>';
	echo '<div class="sbg-delete-all-favorites-block">
		<button data-action="delete-all" class="sbg-delete-all-favorites" type="button">Удалить все записи</button>
	</div>';
}
...
// проверка на сушествование записи
